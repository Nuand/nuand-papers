...
location selected in \ref{sec:dest}, under the \texttt{fx3\_firwmare} folder.
...
the block to open up the block properties. An input for the transmit path may be enabled in the \texttt{TX Configuration}
tab.
...
\begin{itemize}
  \item{Select \texttt{Simulate using: Interpreted Execution} in the block properties \texttt{Devices} tab}
